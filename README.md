<!-- =========================================================================
README de perfil — @pablomtecnologia
Versión completa: estética avanzada + contenido detallado (Actualizado 2026)
========================================================================= -->

<!-- Fondo superior animado -->
<p align="center">
  <img src="https://capsule-render.vercel.app/api?type=waving&color=gradient&height=220&section=header&text=Pablo%20Mart%C3%ADnez%20Fern%C3%A1ndez&fontSize=40&fontColor=fff&fontAlignY=35&animation=twinkling" alt="Header animado"/>
</p>

<!-- Foto personal -->
<p align="center">
  <img src="https://i.imgur.com/iOvwcZX.png" width="210" style="border-radius:50%; border:4px solid #1E90FF;" alt="Foto de Pablo"/>
</p>

<!-- Subtítulo animado -->
<p align="center">
  <img src="https://readme-typing-svg.demolab.com?font=Inter&weight=700&size=22&duration=3000&pause=800&color=1E90FF&center=true&vCenter=true&width=850&lines=Full+Stack+Developer+%7C+Ingeniero+en+formaci%C3%B3n;Sistemas+Cr%C3%ADticos+%7C+Defensa%2C+Aeron%C3%A1utica+y+RF;Desarrollo+de+Firmware+y+Software+en+Indra" alt="Typing animado"/>
</p>

<!-- Badges -->
<p align="center">
  <img src="https://img.shields.io/badge/Visitas-14386-1E90FF?style=for-the-badge"/>
  <img src="https://img.shields.io/badge/Seguidores-420-0A84FF?style=for-the-badge"/>
  <img src="https://img.shields.io/badge/Stars-22-6A5ACD?style=for-the-badge"/>
  <img src="https://img.shields.io/badge/Disponible-Sistemas%20Embebidos%20%7C%20Software-20B2AA?style=for-the-badge"/>
</p>


---

## 👨‍💻 Sobre mí
Soy un **ingeniero en formación y desarrollador**, especializado en el nexo entre el desarrollo de software y los sistemas electrónicos complejos de alta precisión. Actualmente enfocado en el desarrollo de firmware y sistemas críticos para el sector de defensa en **Indra**, cuento con una sólida trayectoria en el **diseño, integración y programación de dispositivos** para aeronáutica, naval, sanidad y administración pública. 

Combino la precisión del hardware industrial, la instrumentación de laboratorio y la validación bajo normativas internacionales (ECSS, MIL-STD-883) con la agilidad en el desarrollo de software empresarial, arquitecturas de backend modernas y automatización avanzada.

---

## 🏢 Experiencia profesional

### 🛰️ **Indra – Desarrollador de Firmware / Sistemas Embebidos** (2025 – Actualidad)
- Desarrollo, integración y optimización de firmware para sistemas críticos de telecomunicaciones y defensa.
- Automatización de pruebas de calibración y análisis de cadenas de transmisión de señal de radiofrecuencia (RF).
- Gestión de instrumentación y optimización de potencia de salida mediante atenuadores dinámicos digitales.
- **Tecnologías:** FPGAs · Python · Xilinx VIO · Entornos Unix/Cygwin · Scripting de testeo automatizado.

### 💻 **LKS NEXT – Programador Full Stack** (2024 – 2025)
- Desarrollo de soluciones digitales en el área de Transformación Digital.  
- Modernización de infraestructuras tecnológicas y optimización de lógica de negocio.  
- **Tecnologías:** Java · Spring Boot · Angular · SQL · Docker.

### ✈️ **Erzia Technologies – Diseño y Construcción de Sistemas RF** (2019 – 2024)
- **Diseño, compra de materiales y construcción de dispositivos electrónicos** complejos para sectores aeronáutico y naval.  
- Programación de sistemas de control y radiofrecuencia.  
- Validación técnica exhaustiva bajo estándares internacionales **ECSS / MIL-STD-883**.  
- Cliente principal: **Hensoldt**.

### 🛡️ **Arquimea – SkyBlock Anti-Drone System** (2022)
- Diseño electrónico, selección de componentes y ensamblaje de un sistema portátil de interferencia táctica contra drones.  
- Construcción de prototipos funcionales, programación de sistemas y validación directa en pruebas de campo.

### 📶 **Clipeo Telecom – Proyectos de Telecomunicaciones** (2017 – 2018)
- Desarrollo y ejecución de planes de medición de campos electromagnéticos de alta precisión para el Ministerio de Industria.  

---

## 🎓 Formación académica
- **Grado en Ingeniería Informática (cursando)**
- **CFGS Desarrollo de Aplicaciones Multiplataforma (DAM)** — 2023–2025  
- **CFGS Mantenimiento Electrónico** — 2021–2023  
- **Grado en Ingeniería Electrónica Industrial y Automática (Etapa)** — Univ. Cantabria  
- **Especialización en Seguridad de Aplicaciones Web (SAW)**  
- **Grado elemental de Piano (Conservatorio Jesús de Monasterio)**

---

## 🚀 Proyectos destacados

### 🛰️ INDRA_STU – Signal Terminal Unit Control (GitHub)
- Repositorio enfocado en la calibración, automatización de tests y control del firmware del módulo **STU** (Signal Terminal Unit).
- Implementación de scripts avanzados para la aproximación precisa de la potencia de salida a $+21\text{ dBm}$ en J2.
- Control dinámico de atenuadores de transmisión (`ATT_TXFI` y `ATT_TXDRIVE`) y monitorización de variables internas mediante analizadores lógicos virtuales.
**Stack:** Python · Scripting · FPGAs (Xilinx VIO) · Firmware · Cygwin.

---

### 🏥 Osakidetza – Aplicaciones hospitalarias internas (2025)
- Desarrollo de **herramientas administrativas internas** para la gestión de hospitales de Osakidetza.  
- Automatización de procesos clínicos internos y tratamiento seguro de datos de carácter sensible.  
- Trabajo centrado en el backend con foco en la escalabilidad y alta disponibilidad de los servicios.  
**Stack:** Java · Spring Boot · SQL · Arquitectura limpia.

---

### 🌍 Sodercan – Plataforma de Ayudas Públicas (2024–2025)
- Creación y optimización de la **web oficial para la tramitación de ayudas públicas en Cantabria**.  
- Programación de formularios dinámicos complejos, validación estructural en tiempo real y gestión de flujos de aprobación seguros.  
**Stack:** Angular · .NET · SQL Server · OAuth2.

---

### 🏀 FantasyBasket – Simulador NBA Fantasy (GitHub)
- Desarrollo completo de un motor de mercado dinámico con recompensas e intercambio de jugadores en tiempo real.  
- Implementación de mecánicas de gamificación basadas en niveles y experiencia de usuario.  
- Integración con **Odoo ERP** a través de servicios REST para la gestión y el canje automático de códigos de recompensa.  
**Stack:** Angular · Spring Boot · Odoo REST API · Docker · JWT · AWS.

---

### 📚 Biblioteca Digital – Gestión de préstamos y devoluciones (GitHub)
- Sistema integral de **préstamos y reserva de activos online** con control automático de plazos de expiración.  
- Integración de módulos de **firma biométrica digital** securizada en los procesos de devolución.  
- Generación automatizada de comprobantes y justificantes en formato PDF dinámico.  
**Stack:** Angular · .NET · SQL Server · iText PDF.

---

### 🏀 SP Basket Pioneers – Web inclusiva (2023–hoy)
- Desarrollo, despliegue y mantenimiento continuo de la plataforma web oficial para una **escuela inclusiva de baloncesto**.  
- Enfoque absoluto en accesibilidad web, usabilidad y soporte técnico.  
**Stack:** HTML · CSS · Angular.

---

### 🛡️ Arquimea – SkyBlock Anti-Drone System (2022)
- Codiseño electrónico y ensamblaje de un rifle portátil para la interferencia y neutralización táctica de drones.  
- Bloqueo efectivo de canales de señal de mando, transmisión de vídeo y sistemas globales de navegación por satélite (GNSS).  
**Stack:** RF · Diseño electrónico · Integración Hardware/Software.

---

## ⚡ Habilidades técnicas

<!-- Fondo dinámico superior -->
<p align="center">
  <img src="https://media3.giphy.com/media/v1.Y2lkPTc5MGI3NjExMWpldnN4NnBuMXl6d2JrZ29wdTF5d2Y3YWN4ZXFhdTlldGh1NnJ4ciZlcD12MV9pbnRlcm5hbF9naWZfYnlfaWQmY3Q9Zw/L8K62iTDkzGX6/giphy.gif" width="100%" height="160" alt="fondo animado"/>
</p>

<div align="center">

### 🔧 Firmware, RF & Sistemas Embebidos
<img src="https://img.shields.io/badge/Xilinx-VIO%20%7C%20Vivado-FF6600?style=flat-square&logo=xilinx&logoColor=white" height="35" alt="Xilinx"/>
<img src="https://img.shields.io/badge/Cygwin-Unix%20Env-00008B?style=flat-square" height="35" alt="Cygwin"/>
<img src="https://img.shields.io/badge/RF%20Sistemas-Aeronáutica%20%7C%20Defensa-1E90FF?style=flat-square" height="35" alt="RF"/>
<img src="https://img.shields.io/badge/FPGA-Desarrollo%20VHDL-8A2BE2?style=flat-square" height="35" alt="FPGA"/>

---

### 💻 Backend  
<img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/java/java-original.svg" height="55" alt="Java"/>
<img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/spring/spring-original.svg" height="55" alt="Spring Boot"/>
<img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/dotnetcore/dotnetcore-original.svg" height="55" alt=".NET"/>
<img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/nodejs/nodejs-original.svg" height="55" alt="Node.js"/>
<img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/python/python-original.svg" height="55" alt="Python"/>
<img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/php/php-original.svg" height="55" alt="PHP"/>

---

### 🎨 Frontend  
<img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/angularjs/angularjs-original.svg" height="55" alt="Angular"/>
<img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/react/react-original.svg" height="55" alt="React"/>
<img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/nextjs/nextjs-original.svg" height="55" alt="Next.js"/>
<img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/typescript/typescript-original.svg" height="55" alt="TypeScript"/>
<img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/tailwindcss/tailwindcss-original.svg" height="55" alt="Tailwind CSS"/>
<img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/bootstrap/bootstrap-original.svg" height="55" alt="Bootstrap"/>

---

### 🗄️ Bases de datos  
<img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/postgresql/postgresql-original.svg" height="55" alt="PostgreSQL"/>
<img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/mysql/mysql-original.svg" height="55" alt="MySQL"/>
<img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/mariadb/mariadb-original.svg" height="55" alt="MariaDB"/>
<img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/mongodb/mongodb-original.svg" height="55" alt="MongoDB"/>
<img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/firebase/firebase-plain.svg" height="55" alt="Firebase"/>

---

### ⚙️ DevOps / Cloud  
<img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/docker/docker-original.svg" height="55" alt="Docker"/>
<img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/github/github-original.svg" height="55" alt="GitHub Actions"/>
<img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/amazonwebservices/amazonwebservices-original.svg" height="55" alt="AWS"/>
<img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/linux/linux-original.svg" height="55" alt="Linux"/>
<br>
<img src="https://img.shields.io/badge/CI%2FCD-Workflow-blue?style=for-the-badge" height="30" alt="CI/CD"/>
<img src="https://img.shields.io/badge/RabbitMQ-Messaging-orange?style=for-the-badge" height="30" alt="RabbitMQ"/>

---

### 📦 Otros  
<img src="https://img.shields.io/badge/Odoo-REST%20API-7F3FBF?logo=odoo&logoColor=white" height="30" alt="Odoo"/>
<img src="https://img.shields.io/badge/iText-PDF-1E90FF" height="30" alt="iText PDF"/>
<img src="https://img.shields.io/badge/Claris-FileMaker-4A90E2" height="30" alt="Claris FileMaker"/>
<img src="https://img.shields.io/badge/Claris-Connect-FF69B4" height="30" alt="Claris Connect"/>

</div>
